...
class VX:
    """
    Veldrix "Royal Governance" palette — cold, desaturated metallic, sampled
...
class VeldrixPageTemplate:
    ...
    def _header(self, c: rl_canvas.Canvas, page_w: float, page_h: float) -> None:
        ...
        c.drawString(lx + ls + 4 * mm, page_h - hh + 9 * mm, "Veldrix")
    ...
    def _footer(self, c: rl_canvas.Canvas, page_w: float, page_h: float, page_num: int) -> None:
        ...
        c.drawString(8 * mm, fy + 2 * mm,
                     "CONFIDENTIAL — Generated by Veldrix · veldrixai.ca · Not for redistribution")
        ...
        c.rotate(35)
        c.drawCentredString(0, 0, "VELDRIX")
        c.restoreState()
...
def _cover(data: dict, s: dict) -> list:
    ...
    story.append(Paragraph(data.get("subtitle", "Deep Research Analysis · Veldrix"), s["cover_sub"]))
    ...
    meta = [
        ...
        ["Tenant / Org",      data.get("tenant", "Veldrix Platform")],
    ...
    hero = Table([[
        ...
        Paragraph(
            ...
            "Weighted average across five Veldrix trust pillars: "
            + ", ".join(
                f"{name} ({w*100:.0f}%)"
    ...
    story.append(Spacer(1, 6 * mm))
    story.append(Paragraph(
        "This report was automatically generated by the Veldrix Runtime Trust Engine. "
        ...
        "using the Veldrix five-pillar evaluation framework. This document is confidential and "
...
def generate_veldrix_pdf(report_data: dict) -> bytes:
    """
    Generate a full branded Veldrix PDF and return it as bytes.
    ...
    tpl = VeldrixPageTemplate(
        ...
        tenant=report_data.get("tenant", "Veldrix Platform"),
    )
    ...
    doc = SimpleDocTemplate(
        ...
        title=f"Veldrix Report — {report_data.get('report_name', '')}",
        author="Veldrix Runtime Trust Engine",
        subject="AI Governance Intelligence Report",
        creator="Veldrix v2.1.0",
    )
    ...
    story.append(Paragraph(
        report_data.get("executive_summary",
            ...
            "evaluated through the Veldrix runtime governance platform. Results reflect "
    ...
    story.append(Paragraph(
        report_data.get("methodology",
            "Veldrix evaluates every AI response through a five-pillar trust framework. "
