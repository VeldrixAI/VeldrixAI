...
function LoginForm() {
  ...
  // OAuth (Google/GitHub) needs configured client IDs/secrets. The local dev
  // mirror has none, so hide the SSO buttons there instead of showing controls
  // that dead-end in an "oauth_not_configured" error. Prod (where creds exist)
  // keeps them. Override with NEXT_PUBLIC_OAUTH_ENABLED.
  const oauthEnabled =
        ...
        <div className="auth-topbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 32px' }}>
          <Link href="/" style={{ display: 'flex', alignItems: 'center', gap: '12px', textDecoration: 'none' }}>
            <div style={{ width: '40px', height: '40px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, background: 'radial-gradient(circle at 50% 45%, rgba(143,166,181,0.18), rgba(143,166,181,0) 70%)', filter: 'drop-shadow(0 0 5px rgba(143,166,181,0.28))' }}>
              <VBrandMark suffix="signin-header" size={40} />
            </div>
        ...
        <div style={{ display: 'flex', flex: 1, minHeight: 'calc(100vh - 80px - 88px)' }}>
          ...
          >
            <div className="glass-panel reveal-up" style={{ width: '100%', maxWidth: '480px', padding: '48px', borderRadius: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.5)', animationDelay: '0.15s' }}>
              ...
              {/* SSO buttons — hidden when OAuth isn't configured (e.g. local dev mirror) */}
              {oauthEnabled && (
                <>
      ...
      <footer style={{ background: 'rgba(10,16,20,0.9)', borderTop: '1px solid rgba(255,255,255,0.05)', padding: '24px 32px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <div style={{ width: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, filter: 'drop-shadow(0 0 4px rgba(143,166,181,0.25))' }}>
            <VBrandMark suffix="signin-footer" size={30} />
          </div>
