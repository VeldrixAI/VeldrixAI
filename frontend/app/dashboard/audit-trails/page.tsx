...
const ACTION_TYPES = ["trust_evaluation", "create_api_key", "revoke_api_key", "login", "logout"];
...
>>>>>>> Stashed changes
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const SYSTEM_TYPES = new Set(["create_report", "delete_report"]);
      const seen = new Set<string>();
      const filtered = (payload.records as AuditRecord[]).filter((r) => {
        if (SYSTEM_TYPES.has(r.action_type)) return false;
        const key = r.request_id || (r.metadata as Record<string, unknown>)?.request_id as string || r.id;
        if (seen.has(key)) return false;
        seen.add(key);
        return true;
      });
      setData({ ...payload, records: filtered });
    } catch (e) {
  ...
  useEffect(() => {
    async function fetchMe() {
      try {
        ...
        if (res.ok) { const d = await res.json(); setUserEmail(d.email || ""); }
      } catch { /* ignore */ }
    }
    fetchMe();
  }, []);
...
>>>>>>> Stashed changes
    try {
      ...
      const allRecords = [...(p1.records || []), ...(p2.records || [])];
      const unique = Array.from(new Map(allRecords.map((r: AuditRecord) => [r.id, r])).values());
      const combined = unique.sort((a: AuditRecord, b: AuditRecord) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      setSystemLogs(combined);
    } catch { /* ignore */ }
  }, []);

  useEffect(() => { loadSystemLogs(); }, [loadSystemLogs]);
  ...
  async function generatePdf(r: AuditRecord, e: React.MouseEvent) {
    ...
    try {
      ...
      if (!res.ok) { const err = await res.json(); throw new Error(err.error || "PDF generation failed"); }
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
  ...
  async function deleteRecord(r: AuditRecord, e: React.MouseEvent) {
    ...
    try {
      ...
      // Trigger immediate reload to update analytics
      setTimeout(() => load(), 100);
    } catch (err) {
...
=======
    const requestId = r.request_id || (m.request_id as string) || (result.request_id as string) || null;
>>>>>>> Stashed changes
    const rawVerdict = (m.verdict as string) || null;
    const riskLevel = (finalScore.risk_level as string) || null;
    const verdict = rawVerdict ? riskToVerdict(rawVerdict) : (riskLevel ? riskToVerdict(riskLevel) : null);
    ...
    return {
      requestId, verdict, overallScore, pillarScores, totalLatencyMs,
    ...
    <>
      <div style={{ padding: "32px", flex: 1, overflowY: "auto" }}>
        <div className="section-reveal" style={{ marginBottom: "32px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <h2 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "32px", letterSpacing: "-1px", color: "#f0f2ff", marginBottom: "6px" }}>Audit Logs</h2>
          ...
          <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
            <div style={{ position: "relative", display: "flex", alignItems: "center" }}>
              ...
              <input type="text" placeholder="Search..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} style={{ paddingLeft: "36px", paddingRight: "14px", paddingTop: "10px", paddingBottom: "10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", color: "#f0f2ff", fontFamily: "DM Sans, sans-serif", fontSize: "13px", outline: "none", width: "180px", transition: "border-color 0.2s" }} onFocus={e => (e.target.style.borderColor = "rgba(124,58,237,0.4)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.07)")} />
            </div>
            <div style={{ position: "relative" }}>
              <select value={actionType} onChange={(e) => { setActionType(e.target.value); setPage(1); }} style={{ padding: "10px 32px 10px 14px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", color: "rgba(240,242,255,0.7)", fontFamily: "DM Sans, sans-serif", fontSize: "13px", outline: "none", appearance: "none", cursor: "pointer", transition: "border-color 0.2s" }} onFocus={e => (e.target.style.borderColor = "rgba(124,58,237,0.4)")} onBlur={e => (e.target.style.borderColor = "rgba(255,255,255,0.07)")}>
            ...
            <button onClick={exportCSV} className="glass-panel" style={{ padding: "10px 18px", borderRadius: "12px", fontFamily: "DM Sans, sans-serif", fontSize: "13px", fontWeight: 600, color: "rgba(240,242,255,0.6)", cursor: "pointer", border: "1px solid rgba(255,255,255,0.07)", display: "flex", alignItems: "center", gap: "8px", background: "none", transition: "all 0.2s" }}>
        ...
        {error && <div style={{ padding: "12px 16px", background: "rgba(244,63,94,0.08)", border: "1px solid rgba(244,63,94,0.2)", borderRadius: "12px", color: "#f43f5e", fontFamily: "DM Sans, sans-serif", fontSize: "13px", marginBottom: "24px" }}>{error}</div>}
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", textAlign: "left", fontFamily: "DM Sans, sans-serif", fontSize: "13px", borderCollapse: "collapse" }}>
              <thead style={{ background: "rgba(255,255,255,0.02)" }}>
                <tr>{["Request ID", "Action", "Verdict", "Trust Score", "Latency", "Timestamp", ""].map((col) => <th key={col} style={{ padding: "12px 20px", fontFamily: "DM Sans, sans-serif", fontWeight: 700, fontSize: "9px", letterSpacing: "2px", textTransform: "uppercase", color: "rgba(240,242,255,0.3)", borderBottom: "1px solid rgba(255,255,255,0.05)", whiteSpace: "nowrap" }}>{col}</th>)}</tr>
              </thead>
              <tbody>
                {loading ? (
                  <tr><td colSpan={7} style={{ padding: "48px", textAlign: "center", color: "rgba(240,242,255,0.3)", fontFamily: "DM Sans, sans-serif", fontSize: "13px" }}>
                  ...
                  <tr><td colSpan={7} style={{ padding: "48px", textAlign: "center", color: "rgba(240,242,255,0.3)", fontFamily: "DM Sans, sans-serif", fontSize: "13px" }}>No audit records yet.</td></tr>
                ) : (
...
<<<<<<< Updated upstream
                      ...
                      >
=======
                      <tr key={r.id} className={`row-in ri-${Math.min(ri + 1, 8)} audit-row`} onClick={() => { if (isSdk && m.requestId) { router.push(`/dashboard/audit-trails/${m.requestId}`); } else { setSelected(r); } }}>
>>>>>>> Stashed changes
                        <td style={{ padding: "14px 20px" }}>
                          {m.requestId ? (
                            <span style={{ display: "inline-flex", alignItems: "center", gap: "6px" }}>
                              <code style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "12px", fontWeight: 700, color: "#7c3aed" }} title={m.requestId}>{m.requestId.slice(0, 8)}</code>
                              <button onClick={(e) => { e.stopPropagation(); copyToClipboard(m.requestId!); }} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(240,242,255,0.3)", padding: "2px", fontSize: "11px" }} onMouseEnter={e => (e.currentTarget.style.color = "#7c3aed")} onMouseLeave={e => (e.currentTarget.style.color = "rgba(240,242,255,0.3)")}>⧉</button>
                            </span>
                          ) : <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "12px", color: "rgba(240,242,255,0.25)" }}>—</span>}
                        </td>
                        <td style={{ padding: "14px 20px" }}><span style={{ padding: "4px 10px", borderRadius: "6px", fontSize: "9px", fontFamily: "DM Sans, sans-serif", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", background: "rgba(124,58,237,0.12)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.2)", whiteSpace: "nowrap" }}>{r.action_type.replace(/_/g, " ")}</span></td>
                        <td style={{ padding: "14px 20px" }}>{vs && m.verdict ? <span style={{ display: "inline-flex", padding: "4px 10px", borderRadius: "6px", fontSize: "9px", fontWeight: 700, letterSpacing: "1.5px", textTransform: "uppercase", background: vs.bg, border: `1px solid ${vs.border}`, color: vs.color, whiteSpace: "nowrap" }}>{m.verdict}</span> : <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "12px", color: "rgba(240,242,255,0.25)" }}>—</span>}</td>
                        <td style={{ padding: "14px 20px" }}>
                          {m.overallScore != null ? (
                            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                              <div style={{ width: "50px", height: "3px", background: "rgba(255,255,255,0.06)", borderRadius: "2px", overflow: "hidden" }}><div style={{ width: `${m.overallScore * 100}%`, height: "100%", background: m.overallScore >= 0.85 ? "#10b981" : m.overallScore >= 0.6 ? "#f59e0b" : "#f43f5e", borderRadius: "2px" }}/></div>
                              <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "11px", fontWeight: 600, color: m.overallScore >= 0.85 ? "#10b981" : m.overallScore >= 0.6 ? "#f59e0b" : "#f43f5e" }}>{(m.overallScore * 100).toFixed(1)}%</span>
                            </div>
                          ) : <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "12px", color: "rgba(240,242,255,0.25)" }}>—</span>}
                        </td>
                        <td style={{ padding: "14px 20px", fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "rgba(240,242,255,0.45)", whiteSpace: "nowrap" }}>{m.totalLatencyMs != null ? `${m.totalLatencyMs}ms` : "—"}</td>
                        <td style={{ padding: "14px 20px", fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "rgba(240,242,255,0.35)", whiteSpace: "nowrap" }}>{fmtShort(r.created_at)}</td>
...
=======
                          <div style={{ display: "flex", gap: "6px" }}>
                            {isSdk && <button onClick={(e) => generatePdf(r, e)} disabled={generatingPdf === reqId || done} className="row-action" style={{ padding: "4px 12px", borderRadius: "7px", fontSize: "10px", fontFamily: "DM Sans, sans-serif", fontWeight: 600, letterSpacing: "1px", border: done ? "1px solid rgba(16,185,129,0.35)" : "1px solid rgba(124,58,237,0.25)", background: "transparent", color: done ? "#10b981" : "#a78bfa", cursor: (generatingPdf === reqId || done) ? "default" : "pointer", opacity: generatingPdf === reqId ? 0.5 : 1, transition: "all 0.2s", whiteSpace: "nowrap" }}>{generatingPdf === reqId ? "Generating…" : done ? "✓ Ready" : "PDF"}</button>}
                            <button className="row-action" onClick={(e) => { e.stopPropagation(); if (isSdk && m.requestId) { router.push(`/dashboard/audit-trails/${m.requestId}`); } else { setSelected(r); } }} style={{ padding: "4px 10px", borderRadius: "7px", fontSize: "10px", fontFamily: "DM Sans, sans-serif", fontWeight: 600, background: "rgba(124,58,237,0.12)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.2)", cursor: "pointer" }}>{isSdk ? "Analyse ↗" : "Open ↗"}</button>
                            <button className="row-action" onClick={(e) => deleteRecord(r, e)} style={{ padding: "4px 10px", borderRadius: "7px", fontSize: "10px", fontFamily: "DM Sans, sans-serif", fontWeight: 600, background: "transparent", color: "#f43f5e", border: "1px solid rgba(244,63,94,0.3)", cursor: "pointer" }}>Delete</button>
>>>>>>> Stashed changes
                          </div>
                        </td>
                      </tr>
          ...
          {data && data.total > 0 && (
            <div style={{ padding: "14px 24px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", background: "rgba(10,12,21,0.3)" }}>
              <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(240,242,255,0.25)" }}>Viewing {(page - 1) * data.limit + 1}–{Math.min(page * data.limit, data.total)} of {data.total.toLocaleString()} records</p>
...
=======
        ...
        <div className="section-reveal" style={{ animationDelay: "0.4s", marginTop: "24px" }}>
          <button onClick={() => setSystemLogsOpen((o) => !o)} style={{ width: "100%", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "14px 20px", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: systemLogsOpen ? "16px 16px 0 0" : "16px", color: "rgba(240,242,255,0.45)", fontFamily: "DM Sans, sans-serif", fontSize: "12px", fontWeight: 600, cursor: "pointer", transition: "all 0.2s" }}>
            <span>{systemLogsOpen ? "↑" : "↓"} System Action Log{systemLogs.length > 0 && <span style={{ marginLeft: 8, padding: "2px 8px", background: "rgba(245,158,11,0.12)", border: "1px solid rgba(245,158,11,0.25)", borderRadius: 6, color: "#f59e0b", fontSize: "10px" }}>{systemLogs.length}</span>}</span>
            <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "10px", letterSpacing: "1px", color: "rgba(240,242,255,0.25)" }}>REPORT_CREATED · REPORT_DELETED</span>
          </button>
          {systemLogsOpen && (
            <div style={{ background: "rgba(10,12,21,0.6)", border: "1px solid rgba(255,255,255,0.05)", borderTop: "none", borderRadius: "0 0 16px 16px", overflow: "hidden" }}>
              {systemLogs.length === 0 ? (
                <div style={{ padding: "32px", textAlign: "center", fontFamily: "DM Sans, sans-serif", fontSize: "13px", color: "rgba(240,242,255,0.2)" }}>No system actions recorded yet.</div>
              ) : (
                <table style={{ width: "100%", textAlign: "left", fontFamily: "DM Sans, sans-serif", fontSize: "13px", borderCollapse: "collapse" }}>
                  <thead><tr>{["Type", "Related Request", "Actor", "Timestamp"].map((col) => <th key={col} style={{ padding: "10px 20px", fontFamily: "DM Sans, sans-serif", fontWeight: 700, fontSize: "9px", letterSpacing: "2px", textTransform: "uppercase", color: "rgba(240,242,255,0.2)", borderBottom: "1px solid rgba(255,255,255,0.04)", whiteSpace: "nowrap" }}>{col}</th>)}</tr></thead>
                  <tbody>
                    {systemLogs.map((r) => {
                      const isCreate = r.action_type === "create_report";
                      const borderColor = isCreate ? "#f59e0b" : "#F43F5E";
                      const relatedId = (r.metadata as Record<string, unknown>)?.source_request_id as string | undefined || (r.metadata as Record<string, unknown>)?.request_id as string | undefined;
                      return (
                        <tr key={r.id} style={{ borderLeft: `3px solid ${borderColor}`, opacity: 0.8 }}>
                          <td style={{ padding: "12px 20px" }}><span style={{ padding: "3px 8px", borderRadius: 6, fontSize: "9px", fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", color: borderColor, background: `${borderColor}14`, border: `1px solid ${borderColor}30`, display: "inline-flex", alignItems: "center", gap: 5 }}>{isCreate ? <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> : <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/></svg>}{isCreate ? "REPORT CREATED" : "REPORT DELETED"}</span></td>
                          <td style={{ padding: "12px 20px" }}>{relatedId ? <button onClick={() => router.push(`/dashboard/audit-trails/${relatedId}`)} style={{ background: "none", border: "none", cursor: "pointer", fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "#7c3aed", padding: 0, textDecoration: "underline" }}>{relatedId.slice(0, 12)}…</button> : <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "rgba(240,242,255,0.2)" }}>—</span>}</td>
                          <td style={{ padding: "12px 20px", fontFamily: "DM Sans, sans-serif", fontSize: "12px", color: "rgba(240,242,255,0.4)" }}>{(r as any).actor || (r.metadata as Record<string, unknown>)?.actor as string || r.ip_address || "—"}</td>
                          <td style={{ padding: "12px 20px", fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "rgba(240,242,255,0.3)", whiteSpace: "nowrap" }}>{fmtShort(r.created_at)}</td>
                        </tr>
                      );
                    })}
                  </tbody>
      ...
      {selected && (() => {
        ...
        return (
          <>
            <div onClick={() => setSelected(null)} style={{ position: "fixed", inset: 0, background: "rgba(5,8,16,0.6)", backdropFilter: "blur(4px)", zIndex: 200 }} />
            <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "420px", background: "#0b0d1c", borderLeft: "1px solid rgba(255,255,255,0.07)", zIndex: 201, display: "flex", flexDirection: "column", overflowY: "auto" }}>
              <div style={{ padding: "24px 28px", borderBottom: "1px solid rgba(255,255,255,0.06)", display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexShrink: 0 }}>
                <div>
                  ...
                  <p style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "rgba(240,242,255,0.3)" }}>{selected.id.slice(0, 24)}…</p>
                </div>
                <button onClick={() => setSelected(null)} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "8px", padding: "8px", cursor: "pointer", color: "rgba(240,242,255,0.5)", display: "flex", alignItems: "center", justifyContent: "center" }} onMouseEnter={e => (e.currentTarget.style.background = "rgba(244,63,94,0.1)")} onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,0.05)")}>
              ...
              <div style={{ padding: "24px 28px", display: "flex", flexDirection: "column", gap: "16px" }}>
                    ...
                    {Object.entries(m.pillarScores).map(([pillar, score]) => {
                      ...
                      return (
                        <div key={pillar} style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "10px" }}>
                          ...
                          <div style={{ flex: 1, height: "3px", background: "rgba(255,255,255,0.06)", borderRadius: "2px", overflow: "hidden" }}><div style={{ width: `${pct}%`, height: "100%", background: color, borderRadius: "2px" }} /></div>
                ...
                {m.criticalFlags.length > 0 && (
                  <div style={{ padding: "14px 16px", background: "rgba(244,63,94,0.06)", border: "1px solid rgba(244,63,94,0.2)", borderRadius: "12px" }}>
                    ...
                    <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "13px", color: "rgba(244,63,94,0.8)", lineHeight: 1.6 }}>{m.criticalFlags.join(" · ")}</div>
                  </div>
                )}
                {selected.action_type === "trust_evaluation" && m.requestId && (
                  <button disabled={generatingPdf === m.requestId || pdfDone.has(m.requestId)} onClick={(e) => generatePdf(selected, e)} style={{ width: "100%", padding: "14px", borderRadius: "12px", border: "none", cursor: "pointer", fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "12px", letterSpacing: "2px", textTransform: "uppercase", background: "linear-gradient(135deg, #9f67ff 0%, #7c3aed 50%, #4f46e5 100%)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", opacity: generatingPdf === m.requestId ? 0.6 : 1 }}>
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg>
                ...
                {selected.metadata && Object.keys(selected.metadata).length > 0 && (
                  <div style={{ background: "#111422", border: "1px solid rgba(255,255,255,0.06)", borderRadius: "14px", padding: "20px" }}>
                    ...
                    <pre style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "11px", color: "rgba(240,242,255,0.5)", whiteSpace: "pre-wrap", wordBreak: "break-all", lineHeight: 1.7, maxHeight: "300px", overflowY: "auto", overflowX: "hidden" }}>{JSON.stringify(selected.metadata, null, 2)}</pre>
                  </div>
                )}
              </div>
            </div>
