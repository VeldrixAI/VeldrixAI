...
const statusMeta: Record<string, { bg: string; text: string; border: string }> = {
  PASSED:      { bg: "rgba(111,169,143,0.1)",  text: "#6fa98f", border: "rgba(111,169,143,0.25)" },
  INTERCEPTED: { bg: "rgba(190,116,104,0.1)",   text: "#be7468", border: "rgba(190,116,104,0.25)" },
  FLAGGED:     { bg: "rgba(194,160,106,0.1)",  text: "#c2a06a", border: "rgba(194,160,106,0.25)" },
};

const riskColor: Record<string, string> = {
  LOW: "#6fa98f", ELEVATED: "#c2a06a", CRITICAL: "#be7468",
};
...
export default function DashboardPage() {
  ...
  const liveFeedItems = auditRows.slice(0, 4).map(mapAuditRow).map((r) => ({
    dot: r.status === "INTERCEPTED" ? "#be7468" : r.status === "FLAGGED" ? "#c2a06a" : "#6fa98f",
    title: r.action,
    body: `${r.model} · ${r.ts}`,
  }));
  ...
  const hudLatencyWidth = hudLatencyMs != null
    ...
    : "0%";
  const hudLatencyColor = hudLatencyMs == null ? "rgba(231,236,239,0.3)"
    : hudLatencyMs <= 100 ? "#6fa98f"
    : hudLatencyMs <= 250 ? "#c2a06a"
    : "#be7468";
  ...
  return (
    <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div className="section-reveal" style={{ marginBottom: "36px", display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: "16px" }}>
          <div>
            <h2 style={{ fontFamily: "Syne, sans-serif", fontWeight: 800, fontSize: "34px", letterSpacing: "-1px", color: "#e7ecef", marginBottom: "8px", lineHeight: 1.1 }}>
              Audit Intelligence
              <span style={{ fontFamily: "DM Sans, sans-serif", fontWeight: 300, fontSize: "18px", color: "rgba(231,236,239,0.4)", marginLeft: "12px", letterSpacing: 0 }}>
                Operations Edition
              </span>
            </h2>
            <p style={{ fontFamily: "DM Sans, sans-serif", fontWeight: 300, fontSize: "14px", color: "rgba(231,236,239,0.5)", maxWidth: "560px", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "flex", gap: "10px", flexShrink: 0 }}>
            <div style={{ display: "inline-flex", border: "1px solid rgba(45,74,94,0.12)", borderRadius: "8px", overflow: "hidden" }}>
              {(["7d", "14d", "30d"] as TimeRange[]).map((r) => (
                <button key={r} onClick={() => setRange(r)} style={{
                  ...
                  background: range === r ? "#2d4a5e" : "rgba(255,255,255,0.03)",
                  color: range === r ? "white" : "rgba(231,236,239,0.4)",
                  ...
                  borderRight: r !== "30d" ? "1px solid rgba(45,74,94,0.12)" : "none",
                }}>
                  {r}
                </button>
              ))}
        ...
        {/* ── Error banner ── */}
        {loadError && (
          <div style={{ padding: "12px 16px", background: "rgba(190,116,104,0.08)", border: "1px solid rgba(190,116,104,0.25)", borderRadius: "12px", color: "#be7468", fontFamily: "DM Sans, sans-serif", fontSize: "13px", marginBottom: "24px" }}>
            {loadError}
          </div>
        )}
          ...
          {loading ? (
            Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="glass-panel" style={{ padding: "22px", borderRadius: "20px", background: "rgba(14,22,26,0.65)" }}>
                <div style={{ height: "10px", width: "60%", borderRadius: "6px", background: "rgba(45,74,94,0.08)", animation: "vx-skeleton 1.4s ease-in-out infinite", marginBottom: "12px" }} />
                <div style={{ height: "28px", width: "45%", borderRadius: "6px", background: "rgba(45,74,94,0.08)", animation: "vx-skeleton 1.4s ease-in-out infinite" }} />
                <div style={{ height: "9px", width: "75%", borderRadius: "6px", background: "rgba(45,74,94,0.08)", animation: "vx-skeleton 1.4s ease-in-out infinite", marginTop: "16px" }} />
              </div>
            ))
          ) : (
            <>
              ...
              <MetricCard
                ...
                valueColor={
                  latencyStatus === "ok" ? "#6fa98f"
                  : latencyStatus === "warn" ? "#c2a06a"
                  : latencyStatus === "critical" ? "#be7468"
        ...
        <div className="section-reveal" style={{ animationDelay: "0.3s", marginBottom: "24px" }}>
          <div style={{ background: "#0a1014", borderRadius: "24px", padding: "28px", border: "1px solid rgba(255,255,255,0.06)", position: "relative", overflow: "hidden" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "28px" }}>
              <div>
                <h4 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: "17px", color: "#e7ecef", marginBottom: "4px" }}>Traffic Velocity</h4>
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 600, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(231,236,239,0.3)" }}>
              ...
              <div style={{ display: "flex", gap: "20px", fontSize: "11px", fontFamily: "DM Sans, sans-serif", fontWeight: 600 }}>
                {[["#2d4a5e", "Incoming"], ["#6fa98f", "Processed"]].map(([col, lbl]) => (
                  <div key={lbl} style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                    ...
                    <span style={{ color: "rgba(231,236,239,0.4)", textTransform: "uppercase", letterSpacing: "1px" }}>{lbl}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div style={{ height: "220px", display: "flex", alignItems: barData.length === 0 ? "center" : "flex-end", justifyContent: barData.length === 0 ? "center" : undefined, gap: "5px", position: "relative" }}>
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(to top, rgba(45,74,94,0.06), transparent)", borderRadius: "8px", pointerEvents: "none" }}/>
              {barData.length === 0 && (
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "12px", color: "rgba(231,236,239,0.2)", letterSpacing: "2px", textTransform: "uppercase", position: "relative" }}>
              ...
              {barData.map((bar, i) => (
                <div
                  ...
                  style={{
                    ...
                    background: `rgba(45,74,94,${bar.op})`,
                    boxShadow: bar.peak ? "0 0 20px rgba(45,74,94,0.4)" : undefined,
                    position: "relative",
                  }}
                >
                  {bar.tip && (
                    <div style={{
                      ...
                      background: "#15222a", border: "1px solid rgba(45,74,94,0.3)",
                      borderRadius: "6px", padding: "3px 8px", fontSize: "10px",
                      fontFamily: "JetBrains Mono, monospace", color: "#e7ecef", whiteSpace: "nowrap",
                    }}>
                      {bar.tip}
                    </div>
                  )}
            ...
            {/* Ambient glow */}
            <div style={{ position: "absolute", bottom: "-60px", right: "-60px", width: "260px", height: "260px", borderRadius: "50%", background: "rgba(45,74,94,0.05)", filter: "blur(80px)", pointerEvents: "none" }}/>
            ...
            {sdkStats && sdkStats.total_requests > 0 && (
              <div style={{ marginTop: "20px", paddingTop: "20px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", gap: "32px", flexWrap: "wrap" }}>
                {[
                  { label: "SDK Requests", val: String(sdkStats.total_requests ?? 0), color: "#2d4a5e" },
                  { label: "Avg Trust Score", val: sdkStats.avg_trust_score != null ? (sdkStats.avg_trust_score * 100).toFixed(1) + "%" : "—", color: sdkStats.avg_trust_score != null && sdkStats.avg_trust_score >= 0.85 ? "#6fa98f" : "#c2a06a" },
                  { label: "SDK Latency", val: sdkStats.avg_latency_ms != null ? `${Math.round(sdkStats.avg_latency_ms)}ms` : "—", color: "#aab8c0" },
                  { label: "Blocked", val: String(sdkStats.verdict_breakdown?.BLOCK ?? 0), color: "#be7468" },
                ].map(({ label, val, color }) => (
                  <div key={label}>
                    <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "9px", fontWeight: 600, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(231,236,239,0.3)", marginBottom: "4px" }}>{label}</div>
        ...
        <section className="section-reveal" style={{ animationDelay: "0.5s" }}>
          <div style={{ background: "#0a1014", borderRadius: "24px", border: "1px solid rgba(255,255,255,0.06)", padding: "28px 32px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "28px" }}>
              <div>
                <h4 style={{ fontFamily: "Syne, sans-serif", fontWeight: 700, fontSize: "16px", color: "#e7ecef", marginBottom: "4px" }}>System Health</h4>
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 600, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(231,236,239,0.3)" }}>Live infrastructure telemetry</p>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <span className="live-dot" style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#6fa98f", display: "inline-block" }}/>
                <span style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "10px", fontWeight: 700, color: "#6fa98f", letterSpacing: "2px" }}>{summary ? "OPERATIONAL" : "CONNECTING…"}</span>
              </div>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "24px", alignItems: "start" }}>
              {/* Column 1: Engine Status */}
              <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(231,236,239,0.25)", marginBottom: "4px" }}>Engine Status</p>
                <HealthBar label="Trust Engine" value={summary ? "ACTIVE" : "—"} valueColor="#6fa98f" width={summary ? "100%" : "20%"} barColor="#6fa98f" delay="hb-1" />
                ...
                <HealthBar
                  ...
                  valueColor={summary?.approval_rate != null && summary.approval_rate >= 90 ? "#6fa98f" : summary?.approval_rate != null && summary.approval_rate >= 70 ? "#c2a06a" : "#aab8c0"}
                  width={summary?.approval_rate != null ? `${Math.round(summary.approval_rate)}%` : "20%"}
                  barColor={summary?.approval_rate != null && summary.approval_rate >= 90 ? "#6fa98f" : "#aab8c0"}
                  delay="hb-3"
                />
                <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: "12px", padding: "16px", display: "flex", alignItems: "center", gap: "16px", marginTop: "4px" }}>
                  <div style={{ display: "flex", gap: "3px", alignItems: "flex-end" }}>
                    {[24, 32, 24, 36, 20].map((h, i) => (
                      <div key={i} className="hbar-fill" style={{ width: "5px", height: `${h}px`, background: summary ? "#6fa98f" : "rgba(255,255,255,0.2)", borderRadius: "2px", opacity: [0.4, 1, 0.7, 0.9, 0.5][i], animationDelay: `${0.3 + i * 0.1}s` }}/>
                    ))}
                  </div>
                  <div>
                    <div style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "10px", fontWeight: 700, color: summary ? "#6fa98f" : "rgba(231,236,239,0.3)", letterSpacing: "2px" }}>ALL SYSTEMS</div>
                    <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "9px", color: "rgba(231,236,239,0.3)", letterSpacing: "1px", textTransform: "uppercase", marginTop: "2px" }}>Operational</div>
                  </div>
                </div>
              </div>
              {/* Column 2: Priority Incident */}
              <div>
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(231,236,239,0.25)", marginBottom: "16px" }}>Priority Incident</p>
                {priorityIncident ? (
                  <div className="glass-panel incident-pulse" style={{ padding: "16px", borderRadius: "14px", background: "rgba(190,116,104,0.05)" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "10px" }}>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="#be7468">
                      ...
                      <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 700, color: "#be7468", letterSpacing: "2px", textTransform: "uppercase" }}>{priorityIncident.status}</span>
                    </div>
                    <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "12px", color: "rgba(231,236,239,0.55)", lineHeight: 1.6, marginBottom: "6px" }}>{priorityIncident.action}</p>
                    <p style={{ fontFamily: "JetBrains Mono, monospace", fontSize: "10px", color: "rgba(231,236,239,0.35)", marginBottom: "14px" }}>{priorityIncident.model} · {priorityIncident.id}</p>
                    <button style={{ width: "100%", padding: "8px", background: "#be7468", color: "white", border: "none", borderRadius: "8px", fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", cursor: "pointer", transition: "opacity 0.2s" }}
                  ...
                  <div style={{ padding: "16px", borderRadius: "14px", background: "rgba(111,169,143,0.05)", border: "1px solid rgba(111,169,143,0.15)", display: "flex", alignItems: "center", gap: "10px" }}>
                    <div style={{ width: 8, height: 8, borderRadius: "50%", background: "#6fa98f", flexShrink: 0 }} className="live-dot" />
                    <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "12px", color: "rgba(231,236,239,0.5)" }}>No active threats detected</span>
                  </div>
                )}
              </div>
              {/* Column 3: Operations Feed */}
              <div>
                <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "10px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(231,236,239,0.25)", marginBottom: "16px" }}>Operations Feed</p>
                {feedItem ? (
                  <>
                    <div style={{ transition: "opacity 0.4s", opacity: feedVisible ? 1 : 0 }}>
                      <div style={{ display: "flex", gap: "10px", alignItems: "flex-start" }}>
                        ...
                        <div>
                          <div style={{ fontFamily: "DM Sans, sans-serif", fontWeight: 600, fontSize: "12px", color: "#e7ecef", marginBottom: "3px" }}>{feedItem.title}</div>
                          <div style={{ fontFamily: "DM Sans, sans-serif", fontSize: "11px", color: "rgba(231,236,239,0.45)", lineHeight: 1.5 }}>{feedItem.body}</div>
                        </div>
                      </div>
                    </div>
                    <div style={{ display: "flex", gap: "6px", marginTop: "16px" }}>
                      {liveFeedItems.map((_, fi) => (
                        <div key={fi} style={{ width: fi === feedIdx % liveFeedItems.length ? "16px" : "6px", height: "6px", borderRadius: "3px", background: fi === feedIdx % liveFeedItems.length ? "#2d4a5e" : "rgba(255,255,255,0.1)", transition: "width 0.3s, background-color 0.3s, opacity 0.3s" }}/>
                      ))}
                    </div>
                  </>
                ) : (
                  <div style={{ display: "flex", alignItems: "center", gap: "10px", padding: "12px 0" }}>
                    <div style={{ width: 7, height: 7, borderRadius: "50%", background: "rgba(231,236,239,0.15)", flexShrink: 0 }} />
                    <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "12px", color: "rgba(231,236,239,0.3)" }}>No recent activity</span>
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: hov ? "0 8px 32px rgba(45,74,94,0.1)" : undefined,
        background: hov ? "rgba(255,255,255,0.04)" : "rgba(14,22,26,0.65)",
      }}
    >
      <div style={{ position: "absolute", top: "12px", right: "12px", opacity: 0.06 }}>
        {bgIcon}
      </div>
      <p style={{ fontFamily: "DM Sans, sans-serif", fontSize: "9px", fontWeight: 600, letterSpacing: "3px", textTransform: "uppercase", color: "rgba(231,236,239,0.35)", marginBottom: "8px" }}>
        {label}
      </p>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px" }}>
        <span style={{ fontFamily: "DM Sans, sans-serif", fontSize: "9px", fontWeight: 700, letterSpacing: "2px", textTransform: "uppercase", color: "rgba(231,236,239,0.35)" }}>{label}</span>
  ...
  return (
    <button
      ...
      style={{
        ...
        color: hov ? "#e7ecef" : "rgba(231,236,239,0.7)", cursor: "pointer",
...
const BigBarChart = () => (
  <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="#2d4a5e" strokeWidth="1.5">
    <path d="M3 3v18h18"/><path d="m19 9-5 5-4-4-3 3"/>
  </svg>
);
const BigAlert = () => (
  <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="#be7468" strokeWidth="1.5">
    <path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"/>
    <line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/>
  </svg>
);
const BigClock = () => (
  <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="rgba(231,236,239,0.5)" strokeWidth="1.5">
    <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
  </svg>
);
const BigShield = () => (
  <svg width="42" height="42" viewBox="0 0 24 24" fill="none" stroke="#6fa98f" strokeWidth="1.5">
    <path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/>
    <polyline points="9 12 11 14 15 10"/>
  </svg>
);
const TrendUp = () => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#6fa98f" strokeWidth="2.5">
    <polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/>
  </svg>
);
const TrendAlert = () => (
  <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#be7468" strokeWidth="2.5">
